...
def process_pending_stt_jobs(guardar_memoria_fn) -> None:
    """Drains async STT jobs and appends results to chat (polling acotado)."""
    pending_stt_jobs = st.session_state.setdefault("pending_stt_jobs", [])
    if not pending_stt_jobs:
        return
    from src.core.async_poll import should_poll_async_jobs

    if not should_poll_async_jobs():
        return
    remaining_stt_jobs = []
...
def render_multimedia_tools_body(
    ...
    *,
    ...
    wp = widget_key_prefix
    if st.button(
        t("multimedia_converter"),
        use_container_width=True,
        key=f"{wp}hub_btn_converter",
    ):
    ...
    audio_stt = st.file_uploader(
        ...
        key=f"{wp}uploader_stt_{st.session_state.form_clear_counter}",
    )
    ...
    if audio_stt:
        ...
        if st.button(t("multimedia_stt_button"), use_container_width=True, key=f"{wp}btn_stt"):
    ...
    with col_voice:
        if prov_tts_sel == t("multimedia_tts_openai"):
            voz_seleccionada = st.selectbox(
                ...
                index=0,
                key=f"{wp}tts_voice_selector",
            )
        else:
            from src.services.audio_service import AVAILABLE_EDGE_VOICES

            voz_alias = st.selectbox(
                t("multimedia_tts_voice"),
                list(AVAILABLE_EDGE_VOICES.keys()),
                key=f"{wp}edge_voice_selector",
            )
    ...
    texto_para_tts = st.text_area(
        ...
        height=120,
        key=f"{wp}tts_input_text_{st.session_state.form_clear_counter}",
    )
    if st.button(t("multimedia_tts_button"), use_container_width=True, key=f"{wp}btn_tts"):
    ...
    proveedor_imagen_sel = st.radio(
        ...
        horizontal=True,
        key=f"{wp}img_provider_radio",
    )
    prompt_imagen_gen = st.text_area(
        t("mm_img_prompt_label"),
        placeholder=t("mm_img_prompt_placeholder"),
        height=80,
        key=f"{wp}img_gen_prompt_{st.session_state.form_clear_counter}",
    )
    k_dalle_size = f"{wp}dalle_size"
    k_dalle_quality = f"{wp}dalle_quality"
    k_stability_aspect = f"{wp}stability_aspect"
    k_stability_negative = f"{wp}stability_negative"
    if proveedor_imagen_sel == "OpenAI DALL-E 3":
        col_size, col_quality = st.columns(2)
        with col_size:
            st.selectbox(
                t("multimedia_image_resolution"),
                ["1024x1024", "1792x1024", "1024x1792"],
                key=k_dalle_size,
            )
        with col_quality:
            st.selectbox(t("multimedia_image_quality"), ["standard", "hd"], key=k_dalle_quality)
    else:
        st.selectbox(
            t("multimedia_image_ratio"),
            ["1:1", "16:9", "9:16", "3:2", "2:3", "4:5", "5:4"],
            key=k_stability_aspect,
        )
        st.text_input(
            t("multimedia_image_negative"),
            placeholder=t("mm_img_negative_placeholder"),
            key=k_stability_negative,
        )
    if st.button(t("multimedia_image_button"), use_container_width=True, key=f"{wp}btn_img_gen"):
            ...
            with st.spinner(t("mm_img_spinner", provider=proveedor_imagen_sel)):
                ...
                if proveedor_imagen_sel == "OpenAI DALL-E 3":
                    filepath_gen, error_gen = generate_image(
                        ...
                        size=st.session_state.get(k_dalle_size, "1024x1024"),
                        quality=st.session_state.get(k_dalle_quality, "standard"),
                    )
                else:
                    filepath_gen, error_gen = generate_image(
                        prompt=prompt_imagen_gen,
                        ...
                        aspect_ratio=st.session_state.get(k_stability_aspect, "1:1"),
                        negative_prompt=st.session_state.get(k_stability_negative, ""),
                    )
            if error_gen:
                st.error(error_gen)
            else:
