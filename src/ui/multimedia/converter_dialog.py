...
def render_converter_dialog(
    ...
    *,
    ...
    archivo_conv = st.file_uploader(
        t("converter_upload"),
        key=f"{widget_key_prefix}uploader_conv_{st.session_state.form_clear_counter}",
    )
    ...
    if archivo_conv:
        ...
        formato_destino = st.text_input(
            t("converter_format"),
            value=st.session_state.get("suggested_format", ""),
            key=f"{widget_key_prefix}converter_format",
        )

        if st.button(
            t("converter_button"),
            use_container_width=True,
            key=f"{widget_key_prefix}converter_submit",
        ):
            if formato_destino:
                try:
                    from src.services.converter_service import normalize_output_extension

                    output_ext = normalize_output_extension(formato_destino)
                ...
                with st.spinner(t("conv_spinner", fmt=formato_destino)):
                    ...
                    output_name = f"conv_{uuid.uuid4().hex[:8]}{output_ext}"
